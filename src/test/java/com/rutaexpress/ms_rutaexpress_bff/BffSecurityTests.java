...
class BffSecurityTests {
    ...
    @ParameterizedTest @ValueSource(strings={"ADMIN","CLIENT"})
    ...
    @ParameterizedTest @ValueSource(strings={"ADMIN","DISPATCHER","CLIENT","AUDITOR"})
    ...
    @ParameterizedTest @ValueSource(strings={"ADMIN","DISPATCHER"})
    ...
    @ParameterizedTest @ValueSource(strings={"CLIENT","AUDITOR"})
    ...
    @ParameterizedTest @ValueSource(strings={"CLIENT","DISPATCHER","ADMIN"})
    void catalogRead(String role) throws Exception {
        token(role);
    ...
    @Test void catalogReadWithoutRoleIsForbidden() throws Exception {
        token();
        mvc.perform(get("/api/catalog/services").header("Authorization","Bearer test-token"))
            .andExpect(status().isForbidden());
        verifyNoInteractions(catalog);
    }
    @ParameterizedTest @ValueSource(strings={"AUDITOR","Admin","Operador","Cliente","Auditor"})
    void catalogReadWithUnsupportedRoleIsForbidden(String role) throws Exception {
        token(role);
        mvc.perform(get("/api/catalog/services").header("Authorization","Bearer test-token"))
            .andExpect(status().isForbidden());
        verifyNoInteractions(catalog);
    ...
    @ParameterizedTest @ValueSource(strings={"CLIENT","DISPATCHER","AUDITOR"})
